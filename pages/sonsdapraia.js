...
export async function getStaticProps() {
  ...
  const resPack = await fetch(`https://freesound.org/apiv2/packs/37158?token=${apiSecret}`)
  const dataPack = await resPack.json();

  const res = await fetch(`https://freesound.org/apiv2/packs/37158/sounds?token=${apiSecret}`)
