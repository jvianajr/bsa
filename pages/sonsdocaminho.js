...
export async function getStaticProps() {
  ...
  const resPack = await fetch(`https://freesound.org/apiv2/packs/37159?token=${apiSecret}`)
  const dataPack = await resPack.json();

  const res = await fetch(`https://freesound.org/apiv2/packs/37159/sounds?token=${apiSecret}`)
