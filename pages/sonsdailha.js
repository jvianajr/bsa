...
export async function getStaticProps() {
  ...
  const resPack = await fetch(`https://freesound.org/apiv2/packs/37157?token=${apiSecret}`)
  const dataPack = await resPack.json();

  const res = await fetch(`https://freesound.org/apiv2/packs/37157/sounds?token=${apiSecret}`)
